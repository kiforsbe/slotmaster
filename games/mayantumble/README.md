...
- **Cascading / Tumbling.** All winning paylines on a grid are totaled together first. Their paths are shown one at a time, then the union of every winning symbol is exploded in one pass. Remaining symbols fall down to close the gaps, and new symbols tumble down from the top to refill the vacated positions. The new grid is then re-evaluated for any new payline wins, cascading repeatedly within the same spin until no new payline wins are formed.
...
Each win draws its own line across the grid, numbered at both ends, one at a time before the
shared explode phase. Ten lines at once would be an unreadable tangle, but none of the symbols
are removed until every winning line has been counted.
