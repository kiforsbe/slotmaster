/**
 * Frequency-tuning pipeline. Simulation execution, numerical optimizers, constraints, and
 * diagnostics are imported through explicit seams so each remains independently testable.
 */
...
import { validateTuningConfig } from './Validation.js';
import { buildLadders, summarize } from './StructuralSensitivity.js';
import { structuralSearch } from './StructuralSearch.js';
import { volatilityBandToSigma, sigmaToVolatilityBand } from './Units.js';
import {
  computeValueRanks, renormalizeWithinBounds, renormalizeWeights, scalePaytable,
  describePayoutScaleVerification, beatsIncumbent,
} from './Payouts.js';
import { gradientDescent1D, bisect1D, nelderMead } from './Optimizers.js';
import { measureSimulationTrials } from '../simulation/TrialMeasurement.js';
/**
...
export async function tuneFrequencies(paytable, reelFrequencyTables, options = {}) {
  ...
  if (dims.length > 0) {
    ...
    // CMA-ES has no initial simplex: with a zero budget it would otherwise enter cmaes() with
    // no sampled population and try to read `best.point`. The baseline above is already a real,
    // seed-stable measurement of the requested starting candidate, so it is the complete and
    // honest result for a zero-budget diagnostic/continuation check.
    if (searchAlgorithm === 'cmaes' && iterationBudget <= 0) {
      return {
        best,
        iterationsUsed: 0,
        restarts: 0,
        stalledOut: false,
        userStopped: false,
        stillImproving: { rtp: false, ordering: false, limits: false, uniformity: false },
        bestOrderingPenalty,
        bestLimitPenalty,
        bestUniformityPenalty,
      };
    }
