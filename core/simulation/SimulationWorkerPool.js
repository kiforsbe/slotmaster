...
function defaultPoolSize() {
  ...
  // More workers than simultaneous candidate trials only add startup, memory and scheduling
  // pressure. Eight keeps modern desktops busy without making a high-core-count machine freeze
  // the developer UI that is reporting progress.
...
export function createSimulationWorkerPool(size = defaultPoolSize()) {
  ...
  function createWorkerEntry() {
    const worker = new Worker(new URL('./trialWorker.js', import.meta.url), { type: 'module' });
  ...
  const workerCount = Math.max(1, Math.min(8, Number.isFinite(size) ? Math.floor(size) : defaultPoolSize()));
  const entries = Array.from({ length: workerCount }, createWorkerEntry);
  ...
  function runTrial(config, numSpins, betPerLine, linesCount, rngSeed) {
    return new Promise((resolve, reject) => {
      if (terminated) {
        reject(new Error('simulation worker pool has been terminated'));
        return;
      }
  ...
  function terminate() {
    if (terminated) return;
    terminated = true;
    const error = new Error('simulation worker pool terminated before the trial completed');
    queue.length = 0;
